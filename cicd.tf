...
resource "aws_codebuild_project" "build_api_gateway" {
  ...
  cache {
    type = "S3"
    location = "${aws_s3_bucket.infra["codebuild-cache"].id}/api-gateway"
  }
  environment {
  ...
  dynamic "vpc_config" {
    for_each = [local.codebuild_vpc_config]
    content {
...
resource "aws_codebuild_project" "test_api_gateway" {
  ...
  description = "Tests for api-gateway"
  build_timeout = 20
  service_role = aws_iam_role.codebuild_api_gateway.arn
  artifacts { type = "CODEPIPELINE" }
  environment {
    ...
    environment_variable {
      name = "DB_CONNECTION"
      value = "/api-gateway/database/connection"
      type = "PARAMETER_STORE"
    }
  }
  source {
    type = "CODEPIPELINE"
    buildspec = local.codebuild_test_buildspec
  }
  logs_config {
    cloudwatch_logs {
      group_name = aws_cloudwatch_log_group.codebuild_api_gateway.name
...
resource "aws_codebuild_project" "build_user_service" {
  ...
  cache {
    type = "S3"
    location = "${aws_s3_bucket.infra["codebuild-cache"].id}/user-service"
  }
  environment {
    compute_type = "BUILD_GENERAL1_MEDIUM"
    image = "aws/codebuild/standard:7.0"
...
resource "aws_codebuild_project" "test_user_service" {
  ...
  environment {
    ...
    environment_variable {
      name = "DB_CONNECTION"
      value = "/user-service/database/connection"
      type = "PARAMETER_STORE"
    }
  }
  source {
...
resource "aws_codebuild_project" "build_order_service" {
  ...
  cache {
    type = "S3"
    location = "${aws_s3_bucket.infra["codebuild-cache"].id}/order-service"
  }
  environment {
    compute_type = "BUILD_GENERAL1_MEDIUM"
    image = "aws/codebuild/standard:7.0"
...
resource "aws_codebuild_project" "test_order_service" {
  ...
  description = "Tests for order-service"
  build_timeout = 20
  service_role = aws_iam_role.codebuild_order_service.arn
  artifacts { type = "CODEPIPELINE" }
  environment {
    ...
    environment_variable {
      name = "DB_CONNECTION"
      value = "/order-service/database/connection"
      type = "PARAMETER_STORE"
    }
  }
  source {
...
resource "aws_codebuild_project" "build_payment_service" {
  ...
  cache {
    type = "S3"
    location = "${aws_s3_bucket.infra["codebuild-cache"].id}/payment-service"
  }
  environment {
    compute_type = "BUILD_GENERAL1_MEDIUM"
    image = "aws/codebuild/standard:7.0"
...
resource "aws_codebuild_project" "test_payment_service" {
  ...
  description = "Tests for payment-service"
  build_timeout = 20
  service_role = aws_iam_role.codebuild_payment_service.arn
  artifacts { type = "CODEPIPELINE" }
  environment {
    ...
    environment_variable {
      name = "DB_CONNECTION"
      value = "/payment-service/database/connection"
      type = "PARAMETER_STORE"
    }
  }
  source {
...
resource "aws_codebuild_project" "build_inventory_service" {
  ...
  description = "Build inventory-service"
  build_timeout = 30
  service_role = aws_iam_role.codebuild_inventory_service.arn
  artifacts { type = "CODEPIPELINE" }
  cache {
    type = "S3"
    location = "${aws_s3_bucket.infra["codebuild-cache"].id}/inventory-service"
  }
  environment {
    compute_type = "BUILD_GENERAL1_MEDIUM"
    image = "aws/codebuild/standard:7.0"
...
resource "aws_codebuild_project" "test_inventory_service" {
  ...
  description = "Tests for inventory-service"
  build_timeout = 20
  service_role = aws_iam_role.codebuild_inventory_service.arn
  artifacts { type = "CODEPIPELINE" }
  environment {
    ...
    environment_variable {
      name = "DB_CONNECTION"
      value = "/inventory-service/database/connection"
      type = "PARAMETER_STORE"
    }
  }
  source {
...
resource "aws_codebuild_project" "build_notification_service" {
  ...
  cache {
    type = "S3"
    location = "${aws_s3_bucket.infra["codebuild-cache"].id}/notification-service"
  }
  environment {
    compute_type = "BUILD_GENERAL1_MEDIUM"
    image = "aws/codebuild/standard:7.0"
...
resource "aws_codebuild_project" "test_notification_service" {
  ...
  description = "Tests for notification-service"
  build_timeout = 20
  service_role = aws_iam_role.codebuild_notification_service.arn
  artifacts { type = "CODEPIPELINE" }
  environment {
    ...
    environment_variable {
      name = "DB_CONNECTION"
      value = "/notification-service/database/connection"
      type = "PARAMETER_STORE"
    }
  }
  source {
...
resource "aws_codebuild_project" "build_catalog_service" {
  name = "catalog-service-build"
  description = "Build catalog-service"
  build_timeout = 30
  service_role = aws_iam_role.codebuild_catalog_service.arn
  artifacts { type = "CODEPIPELINE" }
  cache {
    type = "S3"
    location = "${aws_s3_bucket.infra["codebuild-cache"].id}/catalog-service"
  }
  environment {
    compute_type = "BUILD_GENERAL1_MEDIUM"
    image = "aws/codebuild/standard:7.0"
...
resource "aws_codebuild_project" "test_catalog_service" {
  ...
  description = "Tests for catalog-service"
  build_timeout = 20
  service_role = aws_iam_role.codebuild_catalog_service.arn
  artifacts { type = "CODEPIPELINE" }
  environment {
    ...
    environment_variable {
      name = "DB_CONNECTION"
      value = "/catalog-service/database/connection"
      type = "PARAMETER_STORE"
    }
  }
  source {
...
resource "aws_codebuild_project" "build_search_service" {
  name = "search-service-build"
  description = "Build search-service"
  build_timeout = 30
  service_role = aws_iam_role.codebuild_search_service.arn
  artifacts { type = "CODEPIPELINE" }
  cache {
    type = "S3"
    location = "${aws_s3_bucket.infra["codebuild-cache"].id}/search-service"
  }
  environment {
    compute_type = "BUILD_GENERAL1_MEDIUM"
    image = "aws/codebuild/standard:7.0"
...
resource "aws_codebuild_project" "test_search_service" {
  ...
  environment {
    ...
    environment_variable {
      name = "DB_CONNECTION"
      value = "/search-service/database/connection"
      type = "PARAMETER_STORE"
    }
  }
  source {
...
resource "aws_codepipeline" "api_gateway" {
  ...
  stage {
    ...
    action {
      ...
      configuration = {
        ...
        FullRepositoryId = "myorg/api-gateway"
        BranchName = "main"
      }
    }
  }
  stage {
    ...
    action {
      ...
      configuration = { ProjectName = aws_codebuild_project.test_api_gateway.name }
    }
  }
  stage {
    name = "Build"
    action {
      ...
      configuration = { ProjectName = aws_codebuild_project.build_api_gateway.name }
    }
  }
  stage {
    name = "DeployStaging"
  ...
  stage {
    ...
    action {
      ...
      configuration = {
        ...
        CustomData = "Verify staging of api-gateway"
      }
    }
  }
  stage {
...
resource "aws_codepipeline" "user_service" {
  ...
  stage {
    ...
    action {
      ...
      configuration = {
        ConnectionArn = aws_codestarconnections_connection.github.arn
        FullRepositoryId = "myorg/user-service"
        BranchName = "main"
      }
    }
  }
  stage {
    ...
    action {
      ...
      input_artifacts = ["source_output"]
      output_artifacts = ["test_output"]
      configuration = { ProjectName = aws_codebuild_project.test_user_service.name }
    }
  }
  stage {
    name = "Build"
    action {
      ...
      input_artifacts = ["source_output"]
      output_artifacts = ["build_output"]
      configuration = { ProjectName = aws_codebuild_project.build_user_service.name }
    }
  }
  stage {
    name = "DeployStaging"
  ...
  stage {
    ...
    action {
      ...
      configuration = {
        NotificationArn = aws_sns_topic.main["deployment-events"].arn
        CustomData = "Verify staging of user-service"
      }
    }
  }
  stage {
...
resource "aws_codepipeline" "order_service" {
  ...
  stage {
    ...
    action {
      ...
      input_artifacts = ["source_output"]
      output_artifacts = ["test_output"]
      configuration = { ProjectName = aws_codebuild_project.test_order_service.name }
    }
  }
  stage {
    name = "Build"
    action {
      ...
      input_artifacts = ["source_output"]
      output_artifacts = ["build_output"]
      configuration = { ProjectName = aws_codebuild_project.build_order_service.name }
    }
  }
  stage {
    name = "DeployStaging"
  ...
  stage {
    ...
    action {
      ...
      configuration = {
        NotificationArn = aws_sns_topic.main["deployment-events"].arn
        CustomData = "Verify staging of order-service"
      }
    }
  }
  stage {
...
resource "aws_codepipeline" "payment_service" {
  ...
  stage {
    ...
    action {
      ...
      input_artifacts = ["source_output"]
      output_artifacts = ["test_output"]
      configuration = { ProjectName = aws_codebuild_project.test_payment_service.name }
    }
  }
  stage {
    name = "Build"
    action {
      ...
      input_artifacts = ["source_output"]
      output_artifacts = ["build_output"]
      configuration = { ProjectName = aws_codebuild_project.build_payment_service.name }
    }
  }
  stage {
    name = "DeployStaging"
  ...
  stage {
    ...
    action {
      ...
      configuration = {
        NotificationArn = aws_sns_topic.main["deployment-events"].arn
        CustomData = "Verify staging of payment-service"
      }
    }
  }
  stage {
...
resource "aws_codepipeline" "inventory_service" {
  ...
  stage {
    ...
    action {
      ...
      configuration = {
        ConnectionArn = aws_codestarconnections_connection.github.arn
        FullRepositoryId = "myorg/inventory-service"
        BranchName = "main"
      }
    }
  }
  stage {
    ...
    action {
      ...
      input_artifacts = ["source_output"]
      output_artifacts = ["test_output"]
      configuration = { ProjectName = aws_codebuild_project.test_inventory_service.name }
    }
  }
  stage {
    name = "Build"
    action {
      ...
      input_artifacts = ["source_output"]
      output_artifacts = ["build_output"]
      configuration = { ProjectName = aws_codebuild_project.build_inventory_service.name }
    }
  }
  stage {
    name = "DeployStaging"
  ...
  stage {
    ...
    action {
      ...
      configuration = {
        NotificationArn = aws_sns_topic.main["deployment-events"].arn
        CustomData = "Verify staging of inventory-service"
      }
    }
  }
  stage {
...
resource "aws_codepipeline" "notification_service" {
  ...
  stage {
    ...
    action {
      ...
      configuration = {
        ConnectionArn = aws_codestarconnections_connection.github.arn
        FullRepositoryId = "myorg/notification-service"
        BranchName = "main"
      }
    }
  }
  stage {
    ...
    action {
      ...
      input_artifacts = ["source_output"]
      output_artifacts = ["test_output"]
      configuration = { ProjectName = aws_codebuild_project.test_notification_service.name }
    }
  }
  stage {
    name = "Build"
    action {
      ...
      input_artifacts = ["source_output"]
      output_artifacts = ["build_output"]
      configuration = { ProjectName = aws_codebuild_project.build_notification_service.name }
    }
  }
  stage {
    name = "DeployStaging"
  ...
  stage {
    ...
    action {
      ...
      configuration = {
        NotificationArn = aws_sns_topic.main["deployment-events"].arn
        CustomData = "Verify staging of notification-service"
      }
    }
  }
  stage {
...
resource "aws_codepipeline" "catalog_service" {
  ...
  stage {
    ...
    action {
      ...
      configuration = {
        ConnectionArn = aws_codestarconnections_connection.github.arn
        FullRepositoryId = "myorg/catalog-service"
        BranchName = "main"
      }
    }
  }
  stage {
    ...
    action {
      ...
      input_artifacts = ["source_output"]
      output_artifacts = ["test_output"]
      configuration = { ProjectName = aws_codebuild_project.test_catalog_service.name }
    }
  }
  stage {
    name = "Build"
    action {
      ...
      input_artifacts = ["source_output"]
      output_artifacts = ["build_output"]
      configuration = { ProjectName = aws_codebuild_project.build_catalog_service.name }
    }
  }
  stage {
    name = "DeployStaging"
  ...
  stage {
    ...
    action {
      ...
      configuration = {
        NotificationArn = aws_sns_topic.main["deployment-events"].arn
        CustomData = "Verify staging of catalog-service"
      }
    }
  }
  stage {
...
resource "aws_codepipeline" "search_service" {
  ...
  stage {
    ...
    action {
      ...
      configuration = {
        ConnectionArn = aws_codestarconnections_connection.github.arn
        FullRepositoryId = "myorg/search-service"
        BranchName = "main"
      }
    }
  }
  stage {
    ...
    action {
      ...
      input_artifacts = ["source_output"]
      output_artifacts = ["test_output"]
      configuration = { ProjectName = aws_codebuild_project.test_search_service.name }
    }
  }
  stage {
    name = "Build"
    action {
      ...
      input_artifacts = ["source_output"]
      output_artifacts = ["build_output"]
      configuration = { ProjectName = aws_codebuild_project.build_search_service.name }
    }
  }
  stage {
    name = "DeployStaging"
  ...
  stage {
    ...
    action {
      ...
      configuration = {
        NotificationArn = aws_sns_topic.main["deployment-events"].arn
        CustomData = "Verify staging of search-service"
      }
    }
  }
  stage {
