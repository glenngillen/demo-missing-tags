...
# ============================================================
# RDS — Primary PostgreSQL Instances (1 per env)
# ============================================================

resource "aws_db_instance" "main" {
  for_each = toset(var.environments)

  identifier        = "${each.key}-postgres-main"
  engine            = "postgres"
  engine_version    = "14.10"
  instance_class    = var.rds_instance_classes[each.key]
  allocated_storage = each.key == "prod" ? 500 : 100
  storage_type      = "gp3"
  ...
  parameter_group_name   = aws_db_parameter_group.postgres14[each.key].name

  backup_retention_period   = each.key == "prod" ? 30 : 7
  backup_window             = "03:00-04:00"
  maintenance_window        = "Mon:04:00-Mon:05:00"
  auto_minor_version_upgrade = true
  deletion_protection       = each.key == "prod"
  skip_final_snapshot       = each.key != "prod"
  final_snapshot_identifier = each.key == "prod" ? "${each.key}-postgres-final-snapshot" : null

  performance_insights_enabled          = true
  performance_insights_retention_period = each.key == "prod" ? 731 : 7
